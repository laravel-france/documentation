...
Si vous avez besoin de créer une commande pour un [package workbench](/dev/packages), utilisez l'option `--bench` :
...
Si votre commande est enregistrée dans le [conteneur IoC](/dev/ioc) de votre application, vous pouvez utiliser la méthode `Artisan::resolve` pour la rendre disponible à Artisan :
